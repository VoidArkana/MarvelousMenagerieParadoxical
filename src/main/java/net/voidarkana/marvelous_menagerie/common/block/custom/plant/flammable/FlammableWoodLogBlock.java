package net.voidarkana.marvelous_menagerie.common.block.custom.plant.flammable;
...
public class FlammableWoodLogBlock extends RotatedPillarBlock {
    ...
    public @Nullable BlockState getToolModifiedState(BlockState state, UseOnContext context, ToolAction toolAction, boolean simulate) {
        if (context.getItemInHand().getItem() instanceof AxeItem){
            if(state.is(MMBlocks.SIGILLARIA_STEM.get())){
                return MMBlocks.STRIPPED_SIGILLARIA_STEM.get().defaultBlockState().setValue(AXIS, state.getValue(AXIS));
            }
            ...
            if(state.is(MMBlocks.CALAMITES_BUNDLED_BARK.get())){
                return MMBlocks.STRIPPED_CALAMITES_BUNDLED_BARK.get().defaultBlockState().setValue(AXIS, state.getValue(AXIS));
            }
        }
