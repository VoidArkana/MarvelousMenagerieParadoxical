...
public class Molten extends Monster implements IAnimatedAttacker {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(1, new AnimatedAttackGoal(this, 1.25D,
                true, 12, 18));
...
//            for(int i = 0; i < 2; ++i) {
//                Vec3 vector = new Vec3(this.getRandomX(0.5D), this.getRandomY(), this.getRandomZ(0.5D));
//
//                Vec3 looking = this.getLookAngle();
//
//                vector.add(looking);
//
//                this.level().addParticle(ParticleTypes.LARGE_SMOKE, vector.x, vector.y, vector.z, 0.0D, 0.0D, 0.0D);
//            }
        }
    }
    ...
    static class MoltenShootFireballGoal extends Goal {
        ...
        public boolean canUse() {
            return this.molten.getTarget() != null;
        }
        ...
        public void tick() {
            ...
            if (livingentity != null) {
                ...
                if (livingentity.distanceToSqr(this.molten) < 4096.0D && this.molten.hasLineOfSight(livingentity)) {
                    ...
                    ++this.chargeTime;
                    if (this.chargeTime == 10 && !this.molten.isSilent()) {
                        level.levelEvent((Player)null, 1015, this.molten.blockPosition(), 0);
                    }

                    if (this.chargeTime == 20) {
                        double d1 = 4.0D;
                        Vec3 vec3 = this.molten.getViewVector(1.0F);
                        double d2 = livingentity.getX() - (this.molten.getX() + vec3.x * 4.0D);
                        double d3 = livingentity.getY(0.5D) - (0.5D + this.molten.getY(0.5D));
                        double d4 = livingentity.getZ() - (this.molten.getZ() + vec3.z * 4.0D);
                        if (!this.molten.isSilent()) {
                            level.levelEvent((Player)null, 1016, this.molten.blockPosition(), 0);
                        }

                        SmallFireball smallFireball = new SmallFireball(level, this.molten, d2, d3, d4);
...
//                this.molten.setCharging(this.chargeTime > 10);
            }
        }
    }
