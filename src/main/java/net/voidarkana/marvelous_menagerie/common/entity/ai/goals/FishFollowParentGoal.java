...
public class FishFollowParentGoal extends Goal {
    ...
    private final MarvelousWaterAnimal animal;
    @Nullable
    private MarvelousWaterAnimal parent;
    private final double speedModifier;
    private int timeToRecalcPath;

    public FishFollowParentGoal(MarvelousWaterAnimal pAnimal, double pSpeedModifier) {
        this.animal = pAnimal;
    ...
    public boolean canUse() {
        if (this.animal.getAge() >= 0 || !this.animal.isInWaterOrBubble()) {
            ...
            List<? extends MarvelousWaterAnimal> list = this.animal.level().getEntitiesOfClass(this.animal.getClass(), this.animal.getBoundingBox().inflate(8.0D, 4.0D, 8.0D));
