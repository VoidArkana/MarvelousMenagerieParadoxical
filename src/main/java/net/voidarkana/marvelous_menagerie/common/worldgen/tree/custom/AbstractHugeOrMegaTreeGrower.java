...
public abstract class AbstractHugeOrMegaTreeGrower extends AbstractTreeGrower {
    ...
    public boolean placeHumongous(ServerLevel pLevel, ChunkGenerator pGenerator, BlockPos pPos, BlockState pState, RandomSource pRandom, int pBranchX, int pBranchZ) {
        ...
        if (resourcekey == null) {
            ...
            if (holder == null) {
                ...
                pLevel.setBlock(pPos.offset(pBranchX - 1, 0, pBranchZ - 1), blockstate, 4);
                pLevel.setBlock(pPos.offset(pBranchX - 1, 0, pBranchZ), blockstate, 4);
                pLevel.setBlock(pPos.offset(pBranchX - 1, 0, pBranchZ + 1), blockstate, 4);

                pLevel.setBlock(pPos.offset(pBranchX, 0, pBranchZ -1), blockstate, 4);
                pLevel.setBlock(pPos.offset(pBranchX, 0, pBranchZ), blockstate, 4);
                pLevel.setBlock(pPos.offset(pBranchX, 0, pBranchZ + 1), blockstate, 4);
                ...
                if (configuredfeature.place(pLevel, pGenerator, pRandom, pPos.offset(pBranchX, 0, pBranchZ))) {
                    return true;
                } else {
                    pLevel.setBlock(pPos.offset(pBranchX - 1, 0, pBranchZ - 1), pState, 4);
                    pLevel.setBlock(pPos.offset(pBranchX - 1, 0, pBranchZ), pState, 4);
                    pLevel.setBlock(pPos.offset(pBranchX - 1, 0, pBranchZ + 1), pState, 4);

                    pLevel.setBlock(pPos.offset(pBranchX, 0, pBranchZ -1), pState, 4);
                    pLevel.setBlock(pPos.offset(pBranchX, 0, pBranchZ), pState, 4);
                    pLevel.setBlock(pPos.offset(pBranchX, 0, pBranchZ + 1), pState, 4);
