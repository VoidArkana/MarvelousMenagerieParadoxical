...
public class CraftingRecipeWidget extends BookWidget {
    ...
    public void render(PoseStack poseStack, MultiBufferSource.BufferSource bufferSource, float partialTicks, boolean onFlippingPage) {
        ...
        if(recipe != null){
            ...
            VertexConsumer vertexconsumer = bufferSource.getBuffer(MMRenderTypes.getBookWidget(smelting ? SMELTING_GRID_TEXTURE : CRAFTING_GRID_TEXTURE, sepia));
            poseStack.pushPose();
            ...
            float texHeight;
            if (smelting){
                scaledU1 = 55 / (float)GRID_TEXTURE_SIZE;
                scaledV1 = 37 / (float)GRID_TEXTURE_SIZE;
                texWidth = 55 / 2F;
                texHeight = 37 / 2F;
            }else {
                texWidth = 60 / 2F;
                texHeight = 40 / 2F;
                scaledU1 = 60 / (float)GRID_TEXTURE_SIZE;
                scaledV1 = 40 / (float)GRID_TEXTURE_SIZE;
            }
            ...
            if(smelting){
                ...
                int renderX = 0;
                poseStack.pushPose();
                poseStack.scale(1.025f, 1.025f, 1);
