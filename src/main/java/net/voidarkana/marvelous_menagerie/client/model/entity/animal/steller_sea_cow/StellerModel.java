...
public class StellerModel<T extends StellerSeaCow> extends MarvelousModel<T> {
	...
	public void setupAnim(T entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		...
		this.animateWalk(StellerAnims.SWIM, limbSwing, limbSwingAmount*4f, 1.5f, 3f*entity.getInWaterMultiplier(partialTick));

		this.animateIdle(entity.idleAnimationState, StellerAnims.IDLE, ageInTicks, 1, Math.max(0, entity.getInWaterMultiplier(partialTick)-Math.abs(limbSwingAmount)));
		this.animateIdle(entity.idleAnimationState, StellerAnims.BEACHED, ageInTicks, 1.0F, (1-entity.getInWaterMultiplier(partialTick)));

		this.swim_rot.xRot = Mth.lerp(entity.getInWaterMultiplier(partialTick),0,headPitch * ((float)Math.PI / 180F)/8);
