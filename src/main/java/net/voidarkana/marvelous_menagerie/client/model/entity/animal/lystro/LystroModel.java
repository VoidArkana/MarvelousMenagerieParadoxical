...
public class LystroModel<T extends Lystrosaurus> extends MarvelousModel<T> {
	...
	public void setupAnim(T entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		...
		this.animateWalk(LystroAnims.WALK, limbSwing*1.5f, limbSwingAmount*4f, 4, 2.5f*(1-entity.getSprintingMultiplier(partialTick))*(1-entity.getInWaterMultiplier(partialTick)));
		this.animateWalk(LystroAnims.RUN, limbSwing, limbSwingAmount*4f, 2, entity.getSprintingMultiplier(partialTick)*2.5f*(1-entity.getInWaterMultiplier(partialTick)));

		this.animateIdle(entity.idleAnimationState, LystroAnims.SWIM, ageInTicks, 1.0f, entity.getInWaterMultiplier(partialTick));

		this.animateIdle(entity.idleAnimationState, LystroAnims.RESTING, ageInTicks, 1.0f, Math.max(0, entity.getSittingMultiplier(partialTick)-entity.getInWaterMultiplier(partialTick)-Math.abs(limbSwingAmount)));

		this.animateIdle(entity.idleAnimationState, LystroAnims.IDLE, ageInTicks, 1.0f, Math.max(0, (1-entity.getSittingMultiplier(partialTick))*(1-entity.getInWaterMultiplier(partialTick))-Math.abs(limbSwingAmount)));
