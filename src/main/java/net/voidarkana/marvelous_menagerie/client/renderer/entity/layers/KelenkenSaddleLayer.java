...
@OnlyIn(Dist.CLIENT)
public class KelenkenSaddleLayer<T extends Kelenken> extends RenderLayer<T, MarvelousModel<T>> {

    public KelenkenSaddleLayer(RenderLayerParent<T, MarvelousModel<T>> pRenderer, EntityRendererProvider.Context pContext) {
        super(pRenderer);
    }
    ...
    public void render(PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight, T entityLivingBaseIn, float pLimbSwing, float pLimbSwingAmount, float pPartialTick, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
        ...
        if (entityLivingBaseIn.isSaddled()) {
            ...
            ResourceLocation texture = new ResourceLocation(MarvelousMenagerie.MOD_ID, "textures/entity/animal/kelenken/kelenken_saddle.png");
