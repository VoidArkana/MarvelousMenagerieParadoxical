...
public class ClientLayerRegistry {
    ...
    public static void addLayers(EntityRenderersEvent.AddLayers event) {
        ...
        for (String skinType : event.getSkins()) {
            event.getSkin(skinType).addLayer(new MMSummonGlowLayer(event.getSkin(skinType)));
        }
    }
    ...
    private static void addLayerIfApplicable(EntityType<? extends LivingEntity> entityType, EntityRenderersEvent.AddLayers event) {
        ...
        if (entityType != EntityType.ENDER_DRAGON && entityType != MMEntities.FRACTURE.get() && entityType != MMEntities.RIFT.get()) {
            try {
                ...
                MarvelousMenagerie.LOGGER.warn("Could not apply summon glow layer to " + ForgeRegistries.ENTITY_TYPES.getKey(entityType) + ", has custom renderer that is not LivingEntityRenderer.");
            }
            if (renderer != null) {
                renderer.addLayer(new MMSummonGlowLayer(renderer));
            }
        }
    }
}
